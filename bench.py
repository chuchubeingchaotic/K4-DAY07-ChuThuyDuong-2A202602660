...
Chiến lược cá nhân: Semantic Chunking (chia nhỏ theo ranh giới ngữ nghĩa và cấu trúc điều khoản)
...
from src.chunking import ChunkingStrategyComparator, SemanticChunker
...
def run_benchmark():
    ...
    log("=" * 80)
    log("      LAB 7: BENCHMARK RETRIEVAL TRÊN TẬP DỮ LIỆU CHÍNH SÁCH SHOPEE")
    log("=" * 80)
    log(f"Số lượng tài liệu nạp vào: {len(md_files)}")
    for f in md_files:
        log(f"  - {f.name}")
    ...
    log(f"Tổng số chunk được tạo và lưu trữ: {store.get_collection_size()}")
    for did, count in chunk_counts.items():
        log(f"  * {did}: {count} chunks")
    ...
    log("\n" + "=" * 80)
    log("                 CHẠY 5 CÂU HỎI BENCHMARK CỦA NHÓM")
    log("=" * 80)
    ...
    for item in BENCHMARK_QUERIES:
        ...
        log(f"\n--- Câu hỏi {qid}: {q_text} ---")
        log(f"Gold Answer: {gold} (Nguồn: {src_note})")
        if filt:
            log(f"Metadata filter áp dụng: {filt}")
            results = store.search_with_filter(q_text, top_k=3, metadata_filter=filt)
        else:
            results = store.search(q_text, top_k=3)

        if not results:
            log("  -> Không tìm thấy kết quả phù hợp.")
            continue
        ...
        log(f"Top-1 Chunk ID: {top1['id']} (File: {top1_doc}) | Score: {top1_score:.4f}")
        log(f"Nội dung Top-1: {top1_preview}...")
        log(f"Đánh giá độ liên quan: Top-1 khớp? {'CÓ' if is_top1_relevant else 'KHÔNG'} | Có trong Top-3? {'CÓ' if has_relevant_in_top3 else 'KHÔNG'}")

        agent_ans = agent.answer(q_text, top_k=3)
        log(f"Agent Trả Lời: {agent_ans}")

    log("\n" + "=" * 80)
    log(f"TỔNG KẾT BENCHMARK: {top3_relevant_count}/5 câu hỏi có chunk liên quan trong Top-3")
    log("=" * 80)

    # Ghi kết quả ra file ket_qua_benchmark.txt (Deliverable #3)
    output_file = Path("ket_qua_benchmark.txt")
    output_file.write_text("\n".join(output_lines), encoding="utf-8")
    print(f"\n[OK] Đã lưu kết quả benchmark vào file: {output_file.resolve()}")
